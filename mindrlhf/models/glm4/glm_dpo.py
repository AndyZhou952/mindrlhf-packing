...
from mindformers import ChatGLM2ForConditionalGeneration, ChatGLM2Config
...
class Glm4DPO(ChatGLM2ForConditionalGeneration):
    r"""
    ...
    """
    @lazy_inline
    def __init__(self, config: ChatGLM2Config = None):
        super(Glm4DPO, self).__init__(config)
    ...
    def construct(self, chosen_input_ids, chosen_labels=None, chosen_loss_mask=None, 
        ...
        output = self.transformer(tokens, batch_valid_length=batch_valid_length, block_tables=block_tables, slot_mapping=slot_mapping)
        pre_gather = (not self.use_past or self.is_first_iteration) and batch_valid_length is not None
        if pre_gather:
            output = self.gather(output, self.sub_batch_valid_len(batch_valid_length, 1), 1)
        logits = self.transformer.output_layer(output)
