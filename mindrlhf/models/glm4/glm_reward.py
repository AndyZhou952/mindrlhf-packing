...
from mindformers import ChatGLM2Model, ChatGLM2Config
...
class VHead(nn.Cell):
    ...
    def __init__(self, config=None):
        ...
        mp = 1
        self.v_head0 = Linear(
            in_channels=config.hidden_size, out_channels=1, has_bias=False
...
class Glm4RewardModel(BaseModel):
    r"""
    ...
    def __init__(self, config=None):
        config = config if config is not None else ChatGLM2Config()
        super(Glm4RewardModel, self).__init__(config, auto_prefix=False)
        mp = config.parallel_config.model_parallel
        self.seq_length = config.seq_length
        parallel_config = config.parallel_config
        self.transformer = ChatGLM2Model(config)

        self.v_head0 = VHead(config)
        if parallel_config.pipeline_stage > 1:
            self.v_head0.pipeline_stage = parallel_config.pipeline_stage - 1
            self.transformer.embedding.word_embedding.embedding_table.add_pipeline_stage(
                self.v_head0.pipeline_stage
            )
        ...
        if vocab_size % mp != 0:
            logger.warning(
                "The vocab size of Bloom Loss is: %s, it is not divide by model_parallel: %s",
                vocab_size,
                mp,
            )
            logger.warning(
                "Now, the model_parallel num of Bloom Loss will be changed: mp = 1"
            )
    ...
    def construct(
        ...
        if not self.training:
            return (logits,)
        loss, chosen_end_scores, reject_end_scores = self.loss(
            logits, loss_mask, end_ind
        )
        return loss

    def eval(self, input_ids, end_ind):
        ...
        rejected_rewards = self.slice(
            rewards, (batch_size, 0), (2 * batch_size, seq_length), (1, 1)
        )
    ...
    def infer(self, input_ids, end_ind):
        batch_size, seq_length = F.shape(input_ids)
        output_states = self.transformer(input_ids)
        logits = self.v_head0(output_states)
        logits = logits.squeeze(-1)
